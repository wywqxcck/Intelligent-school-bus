...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagclear)
    {
        bitFieldBitLen += clear_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagclear)
    {
        info->event[info->num] = EVENT_clear;
        info->num++;
        dataPoints->valueclear = gizVarlenDecompressionValue(bitFieldOffset,clear_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += clear_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueaaa != cur->valueaaa)
    {
        GIZWITS_LOG("valueaaa Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagaaa = 1;
        ret = 1;
    }
    if(last->valuebbb != cur->valuebbb)
    {
        GIZWITS_LOG("valuebbb Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagbbb = 1;
        ret = 1;
    }
    if(last->valueccc != cur->valueccc)
    {
        GIZWITS_LOG("valueccc Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagccc = 1;
        ret = 1;
    }
    if(last->valueddd != cur->valueddd)
    {
        GIZWITS_LOG("valueddd Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagddd = 1;
        ret = 1;
    }
    if(last->valueclear != cur->valueclear)
    {
        GIZWITS_LOG("valueclear Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagclear = 1;
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagaaa)
    {
        gizVarlenCompressValue(bitFieldOffset,aaa_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueaaa);
        bitFieldOffset += aaa_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagbbb)
    {
        gizVarlenCompressValue(bitFieldOffset,bbb_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuebbb);
        bitFieldOffset += bbb_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagccc)
    {
        gizVarlenCompressValue(bitFieldOffset,ccc_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueccc);
        bitFieldOffset += ccc_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagddd)
    {
        gizVarlenCompressValue(bitFieldOffset,ddd_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueddd);
        bitFieldOffset += ddd_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagclear)
    {
        gizVarlenCompressValue(bitFieldOffset,clear_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueclear);
        bitFieldOffset += clear_LEN;
    }
